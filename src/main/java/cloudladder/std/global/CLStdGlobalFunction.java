...
//        String str = toString(data);
    ...
    public static void __audio__(CLRtEnvironment env) {
        CLData p1 = env.getOwnVariable("p1").getReferee();
    ...
    @CLBuiltinFuncAnnotation(value={"p1"}, name="text")
    public static void __text__(CLRtEnvironment env) {
        CLData p1 = env.getOwnVariable("p1").getReferee();
        ...
        if (p1 instanceof CLString) {
            ...
            for (CLReference ref : p1.getNumberRefers()) {
                ...
                CLText text = new CLText(path.toString());
                arr.addNumberRefer(text.wrap());
            }
            env.ret(arr.wrap());
        }
    }
    ...
    public static void __save__(CLRtEnvironment env) {
        ...
        if (data instanceof CLImage) {
            ...
            audio.save(path);
        } else if (data instanceof CLText){
            CLText text = (CLText) data;
            text.save(path);
        }
    }
