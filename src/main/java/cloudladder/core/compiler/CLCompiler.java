...
public class CLCompiler {
    @AllArgsConstructor
    @Getter
    private static class ExpressionSpecific {
        private final String varName;
    }

    public ExpressionSpecific compileExpression(ASTExpression tree, CLCompileContext context) {
        if (tree instanceof ASTBinaryExpression) {
            return compileBinaryExpression((ASTBinaryExpression) tree, context);
        } else if (tree instanceof ASTUnaryExpression) {
            return compileUnaryExpression((ASTUnaryExpression) tree, context);
        } else if (tree instanceof ASTBooleanLiteral) {
            return compileBooleanLiteral((ASTBooleanLiteral) tree, context);
        } else if (tree instanceof ASTFieldAccess) {
            return compileFieldAccess((ASTFieldAccess) tree, context);
        } else if (tree instanceof ASTFunctionCall) {
            return compileFunctionCall((ASTFunctionCall) tree, context);
        } else if (tree instanceof ASTIdentifierExpression) {
            return compileIdentifierExpression((ASTIdentifierExpression) tree, context);
        } else if (tree instanceof ASTIndexing) {
            return compileIndexing((ASTIndexing) tree, context);
        } else if (tree instanceof ASTNumberLiteral) {
            return compileNumberLiteral((ASTNumberLiteral) tree, context);
        } else if (tree instanceof ASTStringLiteral) {
            return compileStringLiteral((ASTStringLiteral) tree, context);
        } else if (tree instanceof ASTArrayLiteral) {
            return compileArrayLiteral((ASTArrayLiteral) tree, context);
        } else if (tree instanceof ASTObjLiteral) {
            return compileObjLiteral((ASTObjLiteral) tree, context);
        } else if (tree instanceof ASTFunctionExpression) {
            return compileFunctionExpression((ASTFunctionExpression) tree, context);
        } else if (tree instanceof ASTArrowExpression) {
            return compileArrowExpression((ASTArrowExpression) tree, context);
        } else if (tree instanceof ASTPipeExpression) {
            return compilePipeExpression((ASTPipeExpression) tree, context);
        }

        return null;
    }

    public ExpressionSpecific compileArrowExpression(ASTArrowExpression tree, CLCompileContext context) {
...
//        ExpressionSpecific right = compileExpression(tree.getRight(), context);
    ...
    public ExpressionSpecific compilePipeExpression(ASTPipeExpression tree, CLCompileContext context) {
    ...
    public ExpressionSpecific compileFunctionExpression(ASTFunctionExpression tree, CLCompileContext context) {
    ...
    public ExpressionSpecific compileObjLiteral(ASTObjLiteral tree, CLCompileContext context) {
        String name = "$" + context.iterTempVar();
    ...
    public ExpressionSpecific compileArrayLiteral(ASTArrayLiteral tree, CLCompileContext context) {
    ...
    public ExpressionSpecific compileStringLiteral(ASTStringLiteral tree, CLCompileContext context) {
        String str = tree.getToken().getText();
        str = str.substring(1, str.length() - 1);
        str = StringEscapeUtils.unescapeJava(str);

        int t = context.iterTempVar();
        CLIRDefString ir = new CLIRDefString("$" + t, str);
        context.addIr(ir);

        return new ExpressionSpecific("$" + t);
    }

    public ExpressionSpecific compileNumberLiteral(ASTNumberLiteral tree, CLCompileContext context) {
        int t = context.iterTempVar();
        CLIRDefNumber ir1 = new CLIRDefNumber("$" + t, Double.parseDouble(tree.getToken().getText()));
        context.addIr(ir1);

        return new ExpressionSpecific("$" + t);
    }

    public ExpressionSpecific compileIndexing(ASTIndexing tree, CLCompileContext context) {
        ExpressionSpecific indexer = compileExpression(tree.getIndexer(), context);
        ExpressionSpecific index = compileExpression(tree.getIndex(), context);

        int t = context.iterTempVar();
        CLIRCall ir1 = new CLIRCall("[[access]]", "$" + t);
        ir1.addParam(indexer.getVarName());
    ...
    public ExpressionSpecific compileIdentifierExpression(ASTIdentifierExpression tree, CLCompileContext context) {
        return new ExpressionSpecific(tree.getToken().getText());
    }

    public ExpressionSpecific compileFunctionCall(ASTFunctionCall tree, CLCompileContext context) {
//        String funcName = "";
//        String self = "";
...
//        ir.addParam(self);
    ...
    public ExpressionSpecific compileFieldAccess(ASTFieldAccess tree, CLCompileContext context) {
    ...
    public ExpressionSpecific compileBooleanLiteral(ASTBooleanLiteral tree, CLCompileContext context) {
        String value = tree.getToken().getText();
        return new ExpressionSpecific("$" + value);
    }

    public ExpressionSpecific compileUnaryExpression(ASTUnaryExpression tree, CLCompileContext context) {
     ...
     * @return
     */
    private ExpressionSpecific compileAndExpression(ASTBinaryExpression tree, CLCompileContext context) {
...
//        String dest = "$" + context.iterTempVar();
    ...
    private ExpressionSpecific compileOrExpression(ASTBinaryExpression tree, CLCompileContext context) {
...
//        String dest = "$" + context.iterTempVar();
    ...
    public ExpressionSpecific compileBinaryExpression(ASTBinaryExpression tree, CLCompileContext context) {
        String op = tree.getOp().getText();
        if (op.equals("&&")) {
            return compileAndExpression(tree, context);
        } else if (op.equals("||")) {
            return compileOrExpression(tree, context);
        }
    ...
    public void compileReturnStatement(ASTReturnStatement tree, CLCompileContext context) {
        ExpressionSpecific e1 = compileExpression(tree.getValue(), context);

        CLIRRet ret = new CLIRRet(e1.getVarName());
        context.addIr(ret);
    }
    ...
    public void compileDataStatement(ASTDataStatement tree, CLCompileContext context) {
        for (ASTDataStatementItem item : tree.getItems()) {
            ...
            if (item.getInitializer() != null) {
                ExpressionSpecific e1 = compileExpression(item.getInitializer(), context);
                CLIRRef ir = new CLIRRef(name, e1.getVarName());
                context.addIr(ir);
            } else {
                CLIRRef ir = new CLIRRef(name, null);
                context.addIr(ir);
            }
        }
    }
    ...
    public void compileFile(String program, CLCompileContext context) {
        try {
            ...
            compileProgram((ASTProgram) ast, context);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static CLUtilIRList compileFile(String program) {
        CLCompileContext context = new CLCompileContext();
        CLCompiler compiler = new CLCompiler();
        compiler.compileFile(program, context);

//        context.addIr(new CLIRFileRet());

        return context.getIrList();
    }
